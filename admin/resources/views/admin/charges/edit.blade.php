...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ panel_route('charges.update', $charge) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                                value="{{ old('name', $charge->name) }}" required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="type" id="charge_type" class="form-control @error('type') is-invalid @enderror" required>
                                <option value="fixed" {{ old('type', $charge->type) == 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                                <option value="per_sqft" {{ old('type', $charge->type) == 'per_sqft' ? 'selected' : '' }}>Per Square Feet</option>
                                <option value="per_unit" {{ old('type', $charge->type) == 'per_unit' ? 'selected' : '' }}>Per Unit</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6" id="amount_field" style="{{ $charge->type == 'per_sqft' ? 'display:none;' : '' }}">
                        <div class="mb-3">
                    ...
                    <div class="col-md-6" id="per_sqft_field" style="{{ $charge->type != 'per_sqft' ? 'display:none;' : '' }}">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="per_sqft_rate" class="form-control @error('per_sqft_rate') is-invalid @enderror" 
                                value="{{ old('per_sqft_rate', $charge->per_sqft_rate) }}">
                            @error('per_sqft_rate')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" {{ old('is_active', $charge->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control" rows="3">{{ old('description', $charge->description) }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="text-end">
